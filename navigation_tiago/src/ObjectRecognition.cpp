...
void ObjectRecognition::callback(const darknet_ros_msgs::BoundingBoxesConstPtr& msg)
{
  ...
  // for(int i=0; i<msg->bounding_boxes.size();i++)
  // {
  //   current_name = msg->bounding_boxes[i].Class;
  //   speaktext = "rosrun say_something say.py \" do you want me to grasp the " + current_name + "\"";
  //   const char *command = speaktext.c_str();
  //   system(command);
  //   if(client_1.call(srv))
  //   {
  //     string outcome = srv.response.outcome.text;
  //     string::size_type idx=outcome.find("ye");
  //     ROS_INFO_STREAM(outcome);
  //     if(idx == string::npos)
  //     {
  //       system("rosrun say_something say.py \" lets try another one\"");
  //       continue;
  //     }
  //     else
  //     {
  ...
  //       pub.publish(percep_msg);
  //       speaktext = "rosrun say_something say.py \" Ok, I will grasp it \" ";
  //       system(speaktext.c_str());
  //       while(true)
  //       {
  //         continue;
  //       }
  ...
  // for(int i=0; i<msg->bounding_boxes.size();i++)
  // {
  //   // string names[msg->bounding_boxes.size()];
  //   current_name = msg->bounding_boxes[i].Class;
  //   speaktext = "rosrun say_something say.py \" do you want me to grasp the " + current_name + "\"";
  //   const char *command = speaktext.c_str();
  //   system(command);
  //   client_1.call(srv);
  //   if ((msg->bounding_boxes[i].Class=="cup"||msg->bounding_boxes[i].Class=="bottle")  && bekommen==(bool) 1)
  //   {
  //   percep_msg.x = (msg->bounding_boxes[i].xmin + msg->bounding_boxes[i].xmax)/2;
  //   percep_msg.y = (msg->bounding_boxes[i].ymin + msg->bounding_boxes[i].ymax)/2;
  //   percep_msg.height = (msg->bounding_boxes[i].xmax - msg->bounding_boxes[i].xmin);
  ...
  //   }
    ...
    // if (msg->bounding_boxes[i].Class=="cup" && msg->bounding_boxes[i].Class==a)
    // {
    // percep_msg.x = (msg->bounding_boxes[i].xmin + msg->bounding_boxes[i].xmax)/2;
    // percep_msg.y = (msg->bounding_boxes[i].ymin + msg->bounding_boxes[i].ymax)/2;
    // percep_msg.height = (msg->bounding_boxes[i].xmax - msg->bounding_boxes[i].xmin);
    ...
    // if (msg->bounding_boxes[i].Class=="apple" && msg->bounding_boxes[i].Class==a)
    // {
    // percep_msg.x = (msg->bounding_boxes[i].xmin + msg->bounding_boxes[i].xmax)/2;
    // percep_msg.y = (msg->bounding_boxes[i].ymin + msg->bounding_boxes[i].ymax)/2;
    // percep_msg.height = (msg->bounding_boxes[i].xmax - msg->bounding_boxes[i].xmin);
